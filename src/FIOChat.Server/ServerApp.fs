...
and ServerApp (serverUrl, serverName) =
    ...
    let run serverUrl =
        ...
        let handleCommands server =
            ...
            let handleKickCommand kickedUser msg server =
                fio {
                    ...
                    | true, clientSocket ->
                        ...
                        do! clientSocket.Send <| KickedResponse (server.Name, kickedUser, msg, date)
                        do! broadcastMsg (Some kickedUser) <| DisconnectionNotify (server.Name, kickedUser, $"%s{kickedUser} has been kicked with message: %s{msg}", date)
            ...
            let handleBanCommand bannedUser msg server =
                fio {
                    ...
                    | true, clientSocket ->
                        do! clientSocket.Send <| KickedResponse (server.Name, bannedUser, bannedUserMsg, date)
                        do! broadcastMsg (Some bannedUser) <| DisconnectionNotify (server.Name, bannedUser, clientMsg, date)
                        do! !<< (fun () -> clients.Remove bannedUser |> ignore)
                        do! clientSocket.Close()
                    | false, _ -> 
                        do! printServerMsg date clientMsg
                        do! broadcastMsg (Some bannedUser) <| ServerBroadcastMessageResponse (server.Name, clientMsg, date)
                }
            ...
            let handleUnbanCommand bannedUser msg server =
                fio {
                    ...
                    | true, _ ->
                        ...
                        do! broadcastMsg (Some bannedUser) <| ServerBroadcastMessageResponse (server.Name, msg, date)
                    | false, _ ->
            ...
            let handlePrivateMessageCommand toUser msg server =
                fio {
                    ...
                    | true, clientSocket ->
                        do! clientSocket.Send <| PrivateMessageResponse (server.Name, server.Name, toUser, msg, DateTime.Now)
                    | false, _ ->
            ...
            let handleMessageCommand msg server =
                fio {
                    do! broadcastMsg None <| ServerBroadcastMessageResponse (server.Name, msg, DateTime.Now)
                }
            ...
            let handleConnectionRequest user clientEndPoint =
                fio {
                    ...
                    if user = server.Name then
                        ...
                        do! clientSocket.Send <| ConnectionFailedResponse (server.Name, user, clientMsg, date)
                        do! clientSocket.Close()
                    elif isUserBanned then
                        let! msg = !+ $"%s{user} is banned. Connection denied!"
                        printServerMsg date $"%s{user} is banned. Connection denied!"
                        do! clientSocket.Send <| ConnectionFailedResponse (server.Name, user, msg, date)
                        do! clientSocket.Close()
                    else
            ...
            let handleMsg msg clientEndPoint =
                ...
                fio {
                    ...
                    | BroadcastMessageRequest (fromUser, msg, _) ->
                        ...
                        do! broadcastMsg (Some fromUser) <| BroadcastMessageResponse (serverName, fromUser, msg, date)
                    ...
                    | HelpRequest (fromUser, _) ->
                        ...
                        do! clientSocket.Send <| HelpResponse (server.Name, fromUser, helpMsg, DateTime.Now)
            ...
            let handleDisconnect clientEndPoint =
                fio {
                    ...
                    | Some clientUser ->
                        ...
                        let! date = !+ DateTime.Now
                        do! printServerMsg date serverMsg
                        do! broadcastMsg None <| (DisconnectionNotify (server.Name, serverName, clientMsg, date))
                        do! clientSocket.Close()
                    | None ->
        ...
        fio {
            do! clearConsole()
            let! serverSocket = FServerWebSocket<Message, Message, exn>.Create
