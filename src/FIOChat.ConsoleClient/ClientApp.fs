                ...
                while true do
                    ...
                    try
                        ...
                        | input when input.Trim().StartsWith("\pm@") ->
                            ...
                            do! clientSocket.Send <| PrivateMessageRequest (user, toUser, msg, DateTime.Now)
                        | input when input.Trim().StartsWith("\online") ->
                            do! clientSocket.Send <| OnlineClientsRequest (user, DateTime.Now)
                        | input when input.Trim().StartsWith("\help") ->
                            do! clientSocket.Send <| HelpRequest (user, DateTime.Now)
                        | input ->
                            do! clientSocket.Send <| BroadcastMessageRequest (user, input, DateTime.Now)
                    with exn ->
                        return! !- exn
            }
            ...
            fio {
                ...
                | ConnectionAcceptedResponse (_, acceptedUser, _, _) ->
                    if user <> acceptedUser then
                        return! !- (Exception "Invalid connection response. Incorrect user!")
